...
class ModernTCN(nn.Module):
    def __init__(self,patch_size,patch_stride, stem_ratio, downsample_ratio, ffn_ratio, num_blocks, large_size, small_size, dims, dw_dims,
                 ...
                 subtract_last=False, freq=None, seq_len=512, c_in=7, individual=False, target_window=96, configs=None):
        ...
        if use_multi_scale:
            ...
            self.head = Flatten_Head(self.individual, self.n_vars, self.head_nf, target_window,
                                     head_dropout=head_dropout)
        self.time_proj = TimeSter(time_dim=len(configs.time_feature_types), 
                                                n_variate=configs.c_out, n_out=configs.c_out, seq_len=configs.seq_len, 
                                                pred_len=configs.pred_len, rda=configs.rda, rdb=configs.rdb, 
                                                ksize=configs.ksize)
    ...
    def forward(self, x, te=None):
        ...
        if self.revin:
            ...
            x = self.beta * x + (1 - self.beta) * time_out
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        if self.decomposition:
            ...
            self.model_res = ModernTCN(patch_size=self.patch_size,patch_stride=self.patch_stride,stem_ratio=self.stem_ratio, downsample_ratio=self.downsample_ratio, ffn_ratio=self.ffn_ratio, num_blocks=self.num_blocks, large_size=self.large_size, small_size=self.small_size, dims=self.dims, dw_dims=self.dw_dims,
                 ...
                 subtract_last=self.subtract_last, freq=self.freq, seq_len=self.seq_len, c_in=self.c_in, individual=self.individual, target_window=self.target_window, configs=configs)
    ...
    def forward(self, x, x_mark_enc=None, x_dec=None, y_mark_dec=None):
        ...
        if self.decomposition:
            ...
            res = self.model_res(res_init, x_mark_enc)
            trend = self.model_trend(trend_init, x_mark_enc)
            x = res + trend
            x = x.permute(0, 2, 1)
        else:
