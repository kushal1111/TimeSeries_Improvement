...
class Model(nn.Module):
    ...
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        if 'traffic' in configs.model_id:
            self.time_proj = TimeSter(time_dim=len(configs.time_feature_types), 
                                                    n_variate=configs.c_out, n_out=1, seq_len=configs.seq_len, 
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        time_out = self.time_proj(x_mark_enc)
        dec_out = self.beta * dec_out + (1 - self.beta) * time_out
